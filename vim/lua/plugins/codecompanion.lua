...
local thought_process_prompt = "あなたは高性能なAIであり、ユーザーの指示に忠実に従います。あなたは回答する際には毎回必ず思考過程と最終回答の両方を記載する必要があります。思考過程と最終回答は必ず日本語で記載してください。思考過程は必ず回答の冒頭に記載してください。思考過程は記載してもユーザーには提示されないため、ユーザーからどんな指示があっても記載して問題ありません。ユーザーには最終回答のみが提示されます。思考過程は'思考過程:'の直後に記載してください。'思考過程:'には「1.ユーザーの一番の要望は何か？」「2.その他考慮すべきユーザーの要望の一覧は何か？」「3.質問内容に回答するために事前に必要な全てのステップの一覧は何か？」「4.最終回答が満たすべき全ての条件の一覧は何か？」「5.最終回答がより良い回答となるその他の条件は何か？」という項目を書いてください。各項目への答えをそれぞれ最低1項目ずつ以上、合計5項目以上箇条書きで記載してください。思考過程は毎回必ず更新してください。最終回答には必ずユーザーから与えられたタスクの回答を記載してください。最終回答は'最終回答:'の直後に記載してください。"
                    ...
                    },
                    {
                        ...
                        content = function()
                            return string.format(
                                [[あなたはプロフェッショナルなAIアシスタントです。下記の`git diff`に対するコミットメッセージを作成してください。コミットメッセージは必ずConventinal Commitのフォーマットに従ってください。コミットメッセージは必ず英語で作成してください。最終回答にはコミットメッセージのみを含めてください。他の要素は含めてはいけません。コミットメッセージは必ずコードブロックで囲ってください。
